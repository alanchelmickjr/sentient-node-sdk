 ...
 */
async function streamHelper(responseHandler, eventName, content) {
 ...
 */
async function withErrorHandling(fn, responseHandler) {
    try {
        return await fn();
    }
    catch (error) {
 ...
 */
async function withTimeout(fn, timeoutMs, responseHandler) {
    ...
    try {
        ...
        if (responseHandler && error instanceof Error && error.message.includes('timed out')) {
            await responseHandler.emitError(`Operation timed out after ${timeoutMs}ms`, 408, { timeout: timeoutMs });
            await responseHandler.complete();
        }
        throw error;
    }
 ...
 */
async function withLogging(fn, label = 'Operation') {
 ...
 * @returns Promise that resolves with the function result
 */
async function withRetry(fn, maxRetries = 3, delayMs = 1000, responseHandler) {
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            if (attempt > 1) {
                ...
                if (responseHandler) {
                    await responseHandler.emitTextBlock('RETRY', `Retrying operation (attempt ${attempt}/${maxRetries})...`);
                }
            }
            return await fn();
        }
